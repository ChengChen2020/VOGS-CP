...
def getIntermediateheterFusionDataset(cls):
    ...
    class IntermediateheterFusionDataset(cls):
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            # === Semantic Occupancy ===
            voxel_label_20 = selected_cav_base.get('occ_processed_20', None)
            voxel_label_512 = selected_cav_base.get('occ_processed_512', None)

            semantic_pcd = selected_cav_base.get('occ_raw', None)
            semantic_xyz = None
            semantic_class = None
            if semantic_pcd:
                semantic_xyz = np.asarray(semantic_pcd.points)
                semantic_colors = np.asarray(semantic_pcd.colors)
                semantic_class_raw = (semantic_colors[:, 2] * 255).astype(np.uint8)
                lut = np.zeros(23, dtype=np.int32)
                lut[[1, 2, 5, 6, 7, 8, 9, 10, 11, 12, 14, 15, 17, 18, 20, 22]] = [
                    1, 2, 4, 5, 5, 6, 7, 8, 9, 11, 3, 12, 10, 4, 8, 3
                ]
                semantic_class = lut[semantic_class_raw]
            ...
            label_dict = self.post_processor.generate_label(
                ...
                voxel_label_20=voxel_label_20,
                voxel_label_512=voxel_label_512,
            )
            ...
            if "camera" in sensor_type:
                ...
                for idx, img in enumerate(camera_data_list):
                    camera_to_lidar, camera_intrinsic, lidar2img = self.get_ext_int(params, idx)
                    ...
                    img_src, post_rot2, post_tran2, post_mat = img_transform(
                        img_src,
                        post_rot,
                        post_tran,
                        resize=resize,
                    ...
                    lidar2img_rts.append(torch.from_numpy(lidar2img))
                        ...
                        f"image_inputs_{modality_name}": {
                            ...
                            "image_wh": torch.from_numpy(
                                np.ascontiguousarray(
                                    np.array([img.shape[-2:] for img in imgs], dtype=np.float32)[:, ::-1]
                                    # (H, W) → (W, H)
                                )
                            ), # shape (N, 2)
                            "projection_mat": torch.stack(lidar2img_rts),
                            # pts initialization
                            # "anchor_points": torch.from_numpy(selected_cav_base.get("anchor_points", None)),
            ...
            # === Semantic Occupancy ===
            semantic_xyz_global = None
            if semantic_pcd:
                cav_lidar2global = x_to_world(selected_cav_base["params"]["lidar_pose"])
                ones = np.ones((semantic_xyz.shape[0], 1))
                semantic_xyz_global = (cav_lidar2global @ np.hstack((semantic_xyz, ones)).T).T[:, :3]
            ...
            selected_cav_processed.update(
                {
                    ...
                    # === Semantic Occupancy ===
                    "semantic_xyz_global": semantic_xyz_global,
                    "semantic_class": semantic_class,
                }
            )
        ...
        def __getitem__(self, idx):
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                # 07/16/2025 For vision only, we use camera label
                else:
                ...
                # === Semantic Occupancy ===
                if selected_cav_processed["semantic_class"] is not None:
                    all_labels.append(selected_cav_processed["semantic_class"])
                if selected_cav_processed["semantic_xyz_global"] is not None:
                    all_xyz_global.append(selected_cav_processed["semantic_xyz_global"])
            ...
            # === Semantic Occupancy ===
            if all_labels:  # make sure list is not empty
                all_labels_np = np.concatenate(all_labels, axis=0)
            else:
                all_labels_np = None  # or raise an error, depending on your logic

            if all_xyz_global:
                all_xyz_np = np.concatenate(all_xyz_global, axis=0)
                ones = np.ones((all_xyz_np.shape[0], 1))
                xyz_global_h = np.hstack((all_xyz_np, ones))
                xyz_in_ego = (np.linalg.inv(x_to_world(ego_cav_base["params"]["lidar_pose"])) @ xyz_global_h.T).T[:, :3]
            else:
                xyz_in_ego = None
            # ===
            ...
            # === Semantic Occupancy ===
            voxel_label_20_collab = ego_cav_base.get('occ_processed_20_collab', None)
            voxel_label_512_collab = ego_cav_base.get('occ_processed_512_collab', None)
            ...
            label_dict = self.post_processor.generate_label(
                ...
                # === Semantic Occupancy ===
                semantic_xyz=xyz_in_ego,
                semantic_class=all_labels_np,
                voxel_label_20=voxel_label_20_collab,
                voxel_label_512=voxel_label_512_collab,
            )
