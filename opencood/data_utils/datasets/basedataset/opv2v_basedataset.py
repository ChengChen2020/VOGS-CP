...
class OPV2VBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True):
        ...
        self.load_occ_raw = 'occ_raw' in params['input_source']
        self.load_occ_20 = 'occ_20' in params['input_source']
        self.load_occ_20_collab = 'occ_20_collab' in params['input_source']
        ...
        if self.modality_assignment is not None:
            scenario_folders = [x for x in scenario_folders if os.path.basename(x) in self.modality_assignment.keys()]
    ...
    def reinitialize(self):
        ...
        for (i, scenario_folder) in enumerate(self.scenario_folders):
            ...
            if self.train:
                ...
                cav_list = list(self.modality_assignment[os.path.basename(scenario_folder)].keys())
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                for timestamp in timestamps:
                    ...
                    occ_raw = os.path.join(cav_path, f'{timestamp}_semantic.pcd')
                    occ_raw = occ_raw.replace("OPV2V", "OPV2V/surround")
                    occ_processed_20 = os.path.join(cav_path, f'{timestamp}_voxel_label_20.npy')
                    occ_processed_20_collab = os.path.join(cav_path, f'{timestamp}_voxel_label_20_collab.npy')

                    self.scenario_database[i][cav_id][timestamp].update({
                        'yaml': yaml_file,
                        'lidar': lidar_file,
                        'occ_raw': occ_raw,
                        'occ_processed_20': occ_processed_20,
                        'occ_processed_20_collab': occ_processed_20_collab,
    ...
    def retrieve_base_data(self, idx):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            # Load semantic occupancy (raw Open3D point cloud or cached)
            if self.load_occ_20:
                occ_20_path = cav_content[timestamp_key]['occ_processed_20']
                # if os.path.exists(occ_20_path):
                #     data[cav_id]['occ_processed_20'] = np.load(occ_20_path)
                # else:
                #     data[cav_id]['occ_processed_20'] = occ_20_path
                data[cav_id]['occ_processed_20'] = occ_20_path
            if self.load_occ_20_collab:
                occ_20_collab_path = cav_content[timestamp_key]['occ_processed_20_collab']
                # if os.path.exists(occ_20_collab_path):
                #     data[cav_id]['occ_processed_20_collab'] = np.load(occ_20_collab_path)
                # else:
                #     data[cav_id]['occ_processed_20_collab'] = occ_20_collab_path # STRING
                data[cav_id]['occ_processed_20_collab'] = occ_20_collab_path # STRING
            if self.load_occ_raw:
                data[cav_id]['occ_raw'] = o3d.io.read_point_cloud(cav_content[timestamp_key]['occ_raw'])
    ...
    def generate_object_center_camera(self, 
        ...
        if modality_name is not None:
            ...
            #     cav_contents, reference_lidar_pose, mask_outside_range=mask_outside_range
            # )
            return np.zeros((100, 7)), np.zeros(100), []
    ...
    def get_ext_int(self, params, camera_id):
        ...
        cam2img[:3, :3] = camera_intrinsic
        lidar2img = cam2img @ np.linalg.inv(camera_to_lidar)
        return camera_to_lidar, camera_intrinsic, lidar2img